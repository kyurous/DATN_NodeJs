...
let sendSimpleEmail = async (dataSend) => {
  ...
  let info = await transporter.sendMail({
    from: '"Trung tâm y tế Thành phố Bắc Giang" <ttyttpbacgiang@gmail.com>', // sender address
...
<p>Bạn nhận được email này vì đã đặt lịch khám bệnh online trên website Trung tâm y tế Thành phố Bắc Giang</p>
...
<p>Kết quả khám được gửi trong file đính kèm.</p>
<div>Xin chân thành cảm ơn!</div>
`;
  }
  if (dataSend.language === "en") {
...
let sendCancelBookingEmail = async (dataSend) => {
  ...
  let info = await transporter.sendMail({
    from: '"Trung tâm y tế Thành phố Bắc Giang" <ttyttpbacgiang@gmail.com>',
    to: dataSend.receiverEmail,
    subject: "Thông báo hủy lịch khám bệnh",
...
let getBodyHTMLEmailCancel = (dataSend) => {
  if (dataSend.language === "vi") {
    return `
      <h3><b>Xin chào ${dataSend.patientName}!</b></h3>
      <p>Lịch khám bệnh của bạn đã bị hủy.</p>
      <p><b>Thời gian:</b> ${dataSend.time}</p>
      <p><b>Ngày:</b> ${dataSend.date}</p>
      <p><b>Bác sĩ:</b> ${dataSend.doctorName}</p>
      <div>Xin lỗi vì sự bất tiện này. Vui lòng đặt lại lịch nếu cần.</div>
    `;
  } else {
    return `
      <h3><b>Hello ${dataSend.patientName}!</b></h3>
      <p>Your medical appointment has been canceled.</p>
      <p><b>Time:</b> ${dataSend.time}</p>
      <p><b>Date:</b> ${dataSend.date}</p>
      <p><b>Doctor:</b> ${dataSend.doctorName}</p>
      <div>Sorry for the inconvenience. Please reschedule if necessary.</div>
    `;
  }
};
